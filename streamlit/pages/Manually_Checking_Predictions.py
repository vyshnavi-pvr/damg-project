...
with st.sidebar: 
    st.title("Manually check fraud transactions")
    st.info("This page helps you detect whether a given transaction is fraud or not")
...
if st.button("Submit"):
    ...
    try:
        if dict_rm['predictions'] == "Alert!!! The given transaction seems fraudulent. Check with the higher authority for further process" :
            st.error(dict_rm['predictions'])
        else:
